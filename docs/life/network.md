...
信息技术中心提供的客户端由于未知问题常常导致windows的网络服务注册表损坏，且仅支持windows平台，存在一定局限性。MAC、Linux以及不想使用客户端的Windows用户可以通过自建 L2TP 连接校园网，服务器为 `lns.zju.edu.cn`，账号密码为激活时的账号密码。具体流程可以在参考CC98网友的这篇文章 TODO。
